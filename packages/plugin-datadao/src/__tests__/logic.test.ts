...
describe('Message Handler Logic', () => {
  ...
  it('should register all expected event handlers', () => {
    ...
    requiredEvents.forEach((eventType) => {
      expect(datadaoPlugin.events?.[eventType]).toBeDefined();
      expect(datadaoPlugin.events?.[eventType]?.length).toBeGreaterThan(0);
    });
  });

  it('should process MESSAGE_RECEIVED event and save message to memory', async () => {
    // Get the MESSAGE_RECEIVED handler
    const messageHandler = datadaoPlugin.events?.[EventType.MESSAGE_RECEIVED]?.[0];
...
describe('Reaction Events', () => {
  ...
  it('should store reaction messages correctly', async () => {
    ...
    const reactionHandler = datadaoPlugin.events?.[EventType.REACTION_RECEIVED]?.[0];
...
describe('World and Entity Events', () => {
  ...
  it('should handle ENTITY_JOINED events', async () => {
    ...
    const entityJoinedHandler = datadaoPlugin.events?.[EventType.ENTITY_JOINED]?.[0];
  ...
  it('should handle ENTITY_LEFT events', async () => {
    ...
    const entityLeftHandler = datadaoPlugin.events?.[EventType.ENTITY_LEFT]?.[0];
...
describe('Event Lifecycle Events', () => {
  ...
  it('should handle ACTION_STARTED events', async () => {
    ...
    const actionStartedHandler = datadaoPlugin.events?.[EventType.ACTION_STARTED]?.[0];
  ...
  it('should handle ACTION_COMPLETED events', async () => {
    ...
    const actionCompletedHandler = datadaoPlugin.events?.[EventType.ACTION_COMPLETED]?.[0];
  ...
  it('should handle EVALUATOR_STARTED events', async () => {
    ...
    const evaluatorStartedHandler = datadaoPlugin.events?.[EventType.EVALUATOR_STARTED]?.[0];
  ...
  it('should handle EVALUATOR_COMPLETED events', async () => {
    ...
    const evaluatorCompletedHandler = datadaoPlugin.events?.[EventType.EVALUATOR_COMPLETED]?.[0];
